...
describe('Configuration Utilities', () => {
  ...
  describe('parseConfigContent', () => {
    ...
    it('should throw on invalid JSONC', async () => {
      ...
      await expect(parseConfigContent(content, 'jsonc', 'test.jsonc'))
    ...
    it('should throw on invalid TOML', async () => {
      ...
      await expect(parseConfigContent(content, 'toml', 'test.toml'))
  ...
  describe('findConfigFile', () => {
    ...
    it('should respect file precedence order', async () => {
      ...
      await fs.writeFile(join(tempDir, 'ruleset.config.toml'), 'strict = false');
      await fs.writeFile(join(tempDir, 'ruleset.config.jsonc'), '{"strict": true}');
  ...
  describe('parseEnvOverride', () => {
    it('should parse provider overrides', () => {
      const result = parseEnvOverride('RULESETS_PROVIDERS_CURSOR_ENABLED', 'true');
    ...
    it('should parse output path overrides', () => {
      const result = parseEnvOverride('RULESETS_PROVIDERS_CLAUDE_CODE_OUTPUT_PATH', '/custom/path');
    ...
    it('should parse output directory override', () => {
      const result = parseEnvOverride('RULESETS_OUTPUT_DIRECTORY', '/custom/dist');
