...
OpenAI Codex uses a layered instructions approach with Markdown files to guide the AI agent's behavior.
...
OpenAI Codex uses a two-tier system for managing instructions (highest → lowest precedence):
...
OpenAI Codex rules files use standard Markdown with clear headings and sections:
...
When starting a OpenAI Codex session in a project, the instructions are loaded as follows:

```mermaid
flowchart TD
    A[Start OpenAI Codex in project] --> B[Load ~/.codex/AGENTS.md]
    B --> C{AGENTS.md in project root?}
    C -->|Yes| D[Load project AGENTS.md]
...
## Best Practices for OpenAI Codex Instructions
...
- OpenAI Codex requires an OpenAI API key with sufficient permissions
