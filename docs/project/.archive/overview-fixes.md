...
**Observation:** Variables `{{$key}}` (alias), `{{$.key}}` (frontmatter), and `{{$target}}`/`{{$target.id}}` (system) are generally clear. - The `.` in `{{$.key}}` effectively namespaces frontmatter variables away from aliases. - System variables like `$target` are distinct.
