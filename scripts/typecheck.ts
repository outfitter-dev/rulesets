 ...
 */
async function typecheckPackage(pkgPath: string, options: TypecheckOptions = {}) {
...
async function typecheckAll(options: TypecheckOptions = {}) {
  ...
  if (options.parallel) {
    // Run in parallel
    console.log(`Running type checks in parallel for ${packages.length} packages...\n`);
    ...
    // Run sequentially
    console.log(`Running type checks sequentially for ${packages.length} packages...\n`);
    results = [];
