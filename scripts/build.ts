    ...
    if (options.typecheck !== false) {
      ...
      if (existsSync(tscConfigPath)) {
        try {
          ...
          console.warn(`⚠️  TypeScript declarations failed for ${pkgName} (continuing build)`);
...
async function buildAll(options: BuildOptions = {}) {
  ...
  if (options.parallel) {
    ...
    const groups = [
      ["packages/types"],
      ["packages/parser", "packages/compiler", "packages/linter", "packages/testing"],
      ["packages/plugin-cursor", "packages/plugin-claude-code", "packages/plugin-windsurf"],
      ["packages/core"],
      ["apps/cli"],
    ];
    ...
    for (const group of groups) {
      await Promise.all(
        group.map(pkg => buildPackage(pkg, options))
      );
    }
  } else {
    // Sequential build
