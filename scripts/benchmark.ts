...
async function benchmark() {
  ...
  const incrementalTime = ((incrementalEnd - incrementalStart) / 1000).toFixed(2);
  ...
  const totalTime = parseFloat(cleanBuildTime) + parseFloat(incrementalTime) + 
                    parseFloat(testTime) + parseFloat(typecheckTime);
